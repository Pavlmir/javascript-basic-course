...
function add_cart(value, name) {
    ...
    let product_count = document.querySelector('.' + name);
    let InBasket = true;
    let product = basket.searchProduct(product_name.innerHTML);
    if (product === "") {
        product = new Product(product_name.innerHTML, Number(product_price.innerHTML), product_unit.innerHTML);
        InBasket = false;
    };
    basket.addProduct(product, Number(product_count.value), InBasket);
    let res = basket.getInfoProduct();
    let resCount = res.resCount;
    let resName = res.resName;
    let resSum = basket.countBasketPrice();
    ...
    window.localStorage.basket = json;
